...
class PCcheckTrainer:
    ...
    def __init__(
        ...
        print(f"DRAM RATIO IS {dram_ratio}")
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = PCcheckTrainer.get_optimizer_cls_and_kwargs(
                self.args
            )
        ...
        for epoch in range(1):  # range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if total_batched_samples % args.gradient_accumulation_steps == 0 or (
                    ...
                    print(
                        f"CHFREQ, {self.cfreq}, {steps_since_checkp}, psize {self.psize}, ratio {self.dram_ratio}"
                    )
                    if (step == warmup) or (
                        ...
                        if self.cfreq > 0:
                            if chk_monitor is None:
                                ...
                                gpu_ar, total_size = initialize(
                                    model, [self.optimizer], do_opt_step=False
                                )
                                ...
                                chk_monitor = Chk_monitor(
                                    ...
                                    True,
                                    gpu_ar=gpu_ar,
                                    bsize=total_size // self.psize,
                                    ratio = self.dram_ratio,
                            ...
                            if checkpoints == 1:
                                start_training = time.time()
                        if step == warmup:
                            print(f"Start clock at step {step}!")
                            start_training = time.time()
                    else:
