...
class CheckFreqTrainer:
    ...
    def __init__(
        ...
        self.path_to_pmem = path_to_pmem
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = CheckFreqTrainer.get_optimizer_cls_and_kwargs(self.args)
    ...
    def _inner_training_loop(
        ...
        #chk = CFCheckpoint(model=model.state_dict(), optimizer=self.optimizer.state_dict())
        chk = None
        ...
        for epoch in range(1): #range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if total_batched_samples % args.gradient_accumulation_steps == 0 or (
                    ...
                    if ((step == warmup) or ((self.cfreq > 0) and steps_since_checkp == self.cfreq-1)):
                        if self.cfreq > 0:
                            print("save checkpoint!!!")
                            if chk is None:
                                chk = CFCheckpoint(model=model.state_dict(), optimizer=self.optimizer.state_dict())
                            save_checkpoint("./checkpoint-{epoch}-{it}.chk", self.path_to_pmem,filepath, additional_snapshot, chk, active_snapshot, in_progress_snapshot, lock, \
                ...
                #logger.info(f"Step {step} took {time.time()-starts}")
