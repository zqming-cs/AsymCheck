...
def train(
    ...
    for batch, (data, target, seq_len, _) in enumerate(
        ...
        if (batch == warmup) or (
            ...
            if args.cfreq > 0:
                print(f"Save checkpoint at step {batch},{train_step}")
                ...
                if chk is None:
                    ...
                    # TODO: add remaining state (as above)
                    total_size = get_total_size(model, [optimizer])
                    datasize = total_size * 2 if args.fp16 is True else total_size * 4
                ...
                chk._checkpoint(iter_chk=train_step)
                steps_since_checkp = 0
                checkpoints += 1
            if batch == warmup:
                print(f"Start clock!")
