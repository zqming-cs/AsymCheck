...
def train(
    ...
    chk = (
        None  # CFCheckpoint(model=model.state_dict(), optimizer=optimizer.state_dict())
    )
    ...
    for batch, (data, target, seq_len, _) in enumerate(
        ...
        if (batch == warmup) or (
            ...
            if args.cfreq > 0:
                ...
                if chk is None:
                    chk = CFCheckpoint(state=state)

                # save_checkpoint_script(args, model, mems, model_config, optimizer,
        ...
        print(f"Iteration {batch} took {time.time()-startt}")
        startt = time.time()
