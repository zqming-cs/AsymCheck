...
def main():
    ...
    chunk_size_mb = model_args.chunk_size_mb

    mp.set_start_method("spawn", force=True)
    chk = None
    lock = Lock()
    in_progress_snapshot = Value('i', 0)
    start_receiving = Value('i', 0)
    change = Value('i', 0)
    ...
    for step in range(bench_total_steps):
        ...
        if ((step == warmup) or ((cfreq > 0) and steps_since_checkp == cfreq-1)):
            if (cfreq > 0):
                if chk is None:
                    ...
                    chk = GeminiCheckpoint(gpu_ar, total_size, chunk_size_mb)
                    gemini_receiver = mp.Process(target=create_receiver, args=[
                                                chunk_size_mb, rank, world_size, training_args.fp16, lock, start_receiving])
                    gemini_receiver.start()
                save_checkpoint(chk, rank, world_size,
                                in_progress_snapshot, lock, change)
            if step == warmup:
                print(f"Start clock!")
                start_training = time.time()
            with lock:
                start_receiving.value = 1
        ...
        print(f"Step {step} took {time.time()-start_step} sec")

    if chk is not None:
        gemini_receiver.kill()
        gemini_receiver.join()
