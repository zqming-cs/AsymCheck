...
def main():
    ...
    parser.add_argument("--cfreq", default=1, type=int, help="Checkpoint Frequency")
    parser.add_argument(
        "--path-to-pmem",
        default="",
        type=str,
    ...
    parser.add_argument(
        "--bench_total_steps", default="", type=str, help="Number of steps to train for"
    )
    ...
    if args.do_train:
        ...
        for epoch in range(1):  # range(int(args.num_train_epochs)):
            train_iter = (
                tqdm(
                    train_dataloader,
                    desc="Iteration",
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (args.cfreq > 0) and steps_since_checkp == args.cfreq - 1:
                    ...
                    checkp_start_time = time.time()
                    snapshot = {
                        "model": model.state_dict(),
                        "optimizer": optimizer.state_dict(),
                    }
                    torch.save(snapshot, "checkpoint.pt")
                    with open("checkpoint.pt", "r") as f:
                        f.flush()
                    checkp_end_time = time.time()
                    checkp_time = checkp_end_time - checkp_start_time
                    print(f"Time for a checkpoint is {checkp_time} sec")
                ...
                print(f"Run at step {step}, it took {time.time()-startt}")
                startt = time.time()
