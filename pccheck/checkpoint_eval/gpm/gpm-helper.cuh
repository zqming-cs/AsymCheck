...
#define checkForCudaErrors(ans)                 \
    {                                           \
        gpuAsserter((ans), __FILE__, __LINE__); \
    }
...
static cudaError_t create_gpm_file(const char *path, void **var, size_t size)
{
    ...
    fd = open(path, O_CREAT | O_RDWR | O_TRUNC, (mode_t)0666);

    printf("SET FILE TO SIZE %lu\n", size);
    ftruncate(fd, size);
    if ((*var = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0)) == NULL)
    {
        perror("mmap_file");
        exit(1);
    }
    printf("File mapped, at address %p, fd is %d\n", *var, fd);

    // Map to GPU address space
    printf("------------------ Call cudaHostRegister with size  is %lu\n", size);
    cudaError_t ret = cudaHostRegister(*var, size, cudaHostRegisterDefault);
    printf("------------------ ret is %d\n", ret);
    return ret;
#endif
}
...
static cudaError_t close_gpm_file(const char *path, void *var, size_t size)
{
    ...
    printf("TIME TO UNMAP!\n");
    // pmem_persist(var, size); why?
    if (pmem_unmap(var, 3 * size))
    {
        printf("Error unmapping during file close (%s)\n", pmem_errormsg());
        exit(1);
    }
    msync(var, size, MS_SYNC);
    // Map to GPU address space
    return err;
#endif
}
