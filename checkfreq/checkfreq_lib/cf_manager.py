...
class CFManager:
		...
		def __init__(
				...
				chk_prefix = 'model_v_',
				worker_id = None,
				chk_mode_IOpipeline = False,
    			chk_GPU_stall = False,
       			chk_fullpipeline_stall = False):
				...
				self.chk_mode_IOpipeline = chk_mode_IOpipeline
				self.chk_GPU_stall = chk_GPU_stall
				self.chk_fullpipeline_stall = chk_fullpipeline_stall
				if self.chk_GPU_stall or self.chk_fullpipeline_stall:
		...
		def save_cpu(
			...
			use_thread=True):
				...
				if synchronous:
					if self.chk_mode_IOpipeline:
						self.chk._snapshot_and_persist_async_IOpipeline(filepath, self.active_snapshot, self.in_progress_snapshot, self.lock, snap_ptr, additional_snapshot, iter_chk=self.available_chk_iters, overwrite=self.overwrite)
						self.logger.info("Returned from save in {:.2f}s".format(time.time()-s))
						self.logger.info("[{}] END SAVE".format(time.time()))
					else:
						self.chk._snapshot_and_persist_async(filepath, self.active_snapshot, self.in_progress_snapshot, self.lock, snap_ptr, additional_snapshot, iter_chk=self.available_chk_iters, overwrite=self.overwrite)
						self.logger.info("Returned from save in {:.2f}s".format(time.time()-s))
						self.logger.info("[{}] END SAVE".format(time.time()))
					return 
				...
				if not is_epoch:
					...
					self.chk_process = \
						...
						args=[filepath, self.active_snapshot, self.in_progress_snapshot, self.lock, snap_ptr, additional_snapshot], kwargs=keywords)
				else:
					keywords = { \
						'iter_chk':self.available_chk_iters, \
						'overwrite':self.overwrite, \
		...
		def weight_update(self):
				if 'optimizer' in self.chk.tracking_map:
						...
						s = time.time()
						chk_flag = False
						if self.in_progress_snapshot.value == 1:
							chk_flag = True
						while self.in_progress_snapshot.value == 1:
							continue
   							#self.logger.info("Progresssss")
						if self.chk_fullpipeline_stall and chk_flag:
							with open("./stall.csv", 'a+') as fp_stall:
								fp_stall.write('{}\n'.format(time.time() - s))
						optimizer.step()
